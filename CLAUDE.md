...
- `pnpm e2e:pwa` serves `dist/` itself, like the rest. `pnpm e2e:account`, `pnpm e2e:reset`, `pnpm e2e:google` and `pnpm e2e:admin` need a running server: `URL=http://127.0.0.1:3111 pnpm e2e:account`. Reset, account and admin also need `tests/e2e/smtp-sink.cjs` and `SINK=`, because an address the admin liberates only gets a password through the link; Google needs `tests/e2e/google-sink.cjs`, `GOOGLE=` and, for the off switch check, `DATABASE_URL=`. `tests/e2e/accounts.cjs` holds what they share: becoming the admin, liberating an address and reading the link out of the sink.
...
- Persistence (`src/lib/progress/`): `ProgressState` (version 1) in a row per user, and in localStorage (`lance-a-lance:progress:v1`) only on a page with no API behind it. The puzzle history is a chunked log, 100 entries each, in the same two places (`lance-a-lance:puzzlelog:v1:<chunk>`).
...
- Behind an account the browser keeps no copy: `ProgressProvider` writes to localStorage only when `storageIdentity` is `no-api`, and otherwise holds the puzzle log in a `Map` for the visit. Signing out clears whatever progress keys are left from before this rule, but not the device settings.
- A dead fetch means "no API here" on a static host and "offline" on the real site. `api.ts` remembers the first JSON answer in `lance-a-lance:api:v1` and throws `ApiOffline` after that, which the gate turns into `OfflineScreen`. Do not make the app fall back to localStorage on `ApiOffline`: there is nothing there.
- Accounts (`src/lib/auth/`): `AuthProvider` tells apart "no API behind this page" (a page opened from the file system, or a static host) from "nobody signed in". Signing in or out changes `storageIdentity`, which makes `ProgressProvider` reconnect and reconcile, including every chunk of the puzzle log.
- The app is behind the account: `Gate` in `App.tsx` shows `SignInScreen` while nobody is signed in, `OfflineScreen` when the API cannot be reached, and only then mounts `ProgressProvider`, which waits for the first load before showing the course. `/redefinir` is reachable without a session. The one exception is "no API behind this page", where there is nothing to sign in to and the app runs on localStorage. There is no sync badge: a failed write is caught by the next one or by the load on the next boot.
...
- Passwords use the scrypt in `node:crypto`; sessions are opaque tokens kept only as a SHA-256 hash, in an httpOnly cookie. There is no signing secret by design, so do not add one.
- Signing up is only possible while there are no users: that first account claims the deploy and is the admin (`users.is_admin`). Every other account is created by the admin, by address, through `/api/admin/users` (`server/src/routes/admin.ts`, behind `requireUser` plus an admin check). Such an account has no password and no identity: its owner gets in with Google or by asking for a password on the sign in screen. Google refuses an address nobody liberated, for the same reason.
- The admin can promote, demote and delete, never itself, so the app is never left without an admin. There is no other privilege in the app: `isAdmin` opens `/admin` and nothing else.
- Signing in with Google is a second way in, not a replacement. `server/src/google.ts` builds the authorize URL and trades the code for an identity through the userinfo endpoint (no id token signature to verify). `user_identities` (provider, subject) points at a user, `users.password` is nullable for an account that only has a provider, and `signInWithIdentity` in `auth.ts` is the whole rule: known identity signs in, a new identity whose email Google reports as verified links to the account already holding that email, and otherwise creates one if signup allows. Never link on an unverified email: that is how somebody claims an address they do not own. Refusals redirect to `/?erro=<code>`, which `SignInScreen` turns into a sentence. Without `GOOGLE_CLIENT_ID` the whole thing, button included, is off.
...
- Env: `DATABASE_URL` (required), `PORT`, `COOKIE_SECURE`, `STATIC_DIR`, `APP_URL`, the `SMTP_*` block and `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` (`GOOGLE_AUTH_URL`, `GOOGLE_TOKEN_URL` and `GOOGLE_USERINFO_URL` exist only so the test can point at a fake Google). The `Dockerfile` builds both halves into one image for Easypanel; the container is stateless. The `GIT_SHA` build arg (Easypanel passes it) becomes `__APP_VERSION__` in the bundle, which the home footer shows; it is a build arg, not a runtime variable.
- Local: start a Postgres, then `DATABASE_URL=... COOKIE_SECURE=false pnpm dev:server`.
- The app answers `/`, `/redefinir?token=...` and `/admin`, handled in `App.tsx`, not by a router. Everything else is a 404 from the server (`public/404.html`). That list lives in three places that must stay in step: `CLIENT_ROUTES` in `server/src/index.ts`, `navigateFallbackAllowlist` in `vite.config.ts` (or the service worker answers unknown addresses with the shell) and the routing in `App.tsx`.
