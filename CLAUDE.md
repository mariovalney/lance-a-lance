...
- `pnpm e2e:pwa` serves `dist/` itself, like the rest. `pnpm e2e:account`, `pnpm e2e:reset` and `pnpm e2e:google` need a running server: `URL=http://127.0.0.1:3111 pnpm e2e:account`. Reset also needs `tests/e2e/smtp-sink.cjs` and `SINK=`; Google needs `tests/e2e/google-sink.cjs`, `GOOGLE=` and, for the off switch check, `DATABASE_URL=`.
...
- Env: `DATABASE_URL` (required), `PORT`, `SIGNUP_ENABLED`, `COOKIE_SECURE`, `STATIC_DIR`, `APP_URL`, the `SMTP_*` block and `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` (`GOOGLE_AUTH_URL`, `GOOGLE_TOKEN_URL` and `GOOGLE_USERINFO_URL` exist only so the test can point at a fake Google). The `Dockerfile` builds both halves into one image for Easypanel; the container is stateless. The `GIT_SHA` build arg (Easypanel passes it) becomes `__APP_VERSION__` in the bundle, which the home footer shows; it is a build arg, not a runtime variable.
