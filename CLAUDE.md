...
- The local copy is tagged with the account that owns it (`lance-a-lance:owner:v1`). Signing out clears the progress, the puzzle log and the tag, but not the device settings; an account that finds another account's tag starts from its own cloud copy instead of adopting what is here. An untagged copy is progress made before there was an account, so the first account to sign in adopts it.
...
- The app is behind the account: `Gate` in `App.tsx` shows `SignInScreen` while nobody is signed in, and only then mounts `ProgressProvider`. `/redefinir` is reachable without one. The one exception is "no API behind this page", where there is nothing to sign in to and the app runs on localStorage. There is no sync badge: a failed cloud write is caught by the next one or by the reconcile on the next boot.
